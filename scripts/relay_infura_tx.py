...
parser.add_argument(
    "safe_tx_hash", type=str, help="Safe tx hash to get from tx service"
)
...
if response.ok:
        ...
        b"".join(
            [
                ...
                for confirmation in sorted(
                    data["confirmations"], key=lambda x: x["owner"].lower()
                )
            ]
        )
    )
    ...
    ethereum_client = EthereumClient(
        "https://staging-openethereum.rinkeby.gnosisdev.com/"
    )
    safe_tx = SafeTx(
        ethereum_client,
        data["safe"],
    ...
    tx_data = safe_tx.w3_tx.buildTransaction(
        {"gas": 0, "from": Account.create().address}
    )["data"]
    payload = {
