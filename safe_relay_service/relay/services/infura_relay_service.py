...
class InfuraRelayServiceProvider:
    def __new__(cls):
        if not hasattr(cls, "instance"):
            ...
            cls.instance = InfuraRelayService(
                settings.INFURA_NODE_URL, settings.INFURA_RELAY_SENDER_PRIVATE_KEY
            )
        return cls.instance
...
class InfuraRelayService:
    REFUNDER_ADDRESSES = {
        EthereumNetwork.RINKEBY: ChecksumAddress(
            "0x0971BF033F429B6077b81911505c406Ffb8cde2c"
        ),
    }

    def __init__(self, infura_node_url: str, infura_relay_sender_private_key: str):
        self.infura_node_url = infura_node_url
        self.infura_relay_sender_account = Account.from_key(
            infura_relay_sender_private_key
        )
    ...
    def build_refunder_transaction_data(
        self, to: ChecksumAddress, data: bytes
    ) -> HexBytes:
        return HexBytes(
            self.refunder_contract.functions.executeTrusted(to, data).buildTransaction(
                {"gas": 0, "gasPrice": 0}
            )["data"]
        )
    ...
    def estimate_gas(self, to: ChecksumAddress, data: bytes):
        ...
        """
        try:
            return self.ethereum_client.w3.eth.estimateGas(
                {
                    "to": to,
            ...
            #              'data': 'Reverted'}
            raise InfuraRelayServiceException(
                f"Cannot estimate gas price for tx to={to} data={data.hex()}"
            ) from exc
    ...
    def send_transaction(self, to: ChecksumAddress, data: bytes) -> InfuraTxSent:
        ...
        gas = self.estimate_gas(
            self.refunder_contract.address, refunder_transaction_data
        )
        itx_relay_tx = ItxRelayTx(
            self.refunder_contract.address, refunder_transaction_data, gas * 2
        )
        infura_tx_hash = self.itx_client.send_transaction(
            itx_relay_tx, self.infura_relay_sender_account
        )
        ...
        if transaction_status and (broadcasts := transaction_status.get("broadcasts")):
            ...
            broadcasts_sorted = sorted(
                ...
                key=lambda d: datetime.strptime(
                    d["broadcast_time"], "%Y-%m-%dT%H:%M:%S.%fZ"
                ),
                reverse=True,
            )
